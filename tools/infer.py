...
def load_image(path, transform):
    ...
    # ToTensorV2 on grayscale gives (1, H, W)
    if result.ndim == 2:
        result = result.unsqueeze(0)
    return result.unsqueeze(0)  # (1, 1, H, W)


def run_single(model, transform, decoder, device, path, threshold, out_file):
    img = load_image(path, transform).to(device)
    with torch.no_grad():
        pred = model(img)
    ...
    out_file.write(f"{name} {text} {conf:.3f}\n")
    ...
    with open(save_path, "w") as f:
        if os.path.isdir(image_path):
            for name in sorted(os.listdir(image_path)):
                ...
                try:
                    run_single(model, transform, decoder, device, p, threshold, f)
                except RuntimeError as e:
                    print(f"Skipping {name}: {e}")
        else:
            run_single(model, transform, decoder, device, image_path, threshold, f)
